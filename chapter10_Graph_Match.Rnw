...
\subsection{Fast approximate quadratic programming with smooth transition from rQAP2 to rQAP}
...
$-2*(A_{21}B_{21}^T+A_{12}^TB_{12})+A_{22}PB_{22}^T+A_{22}^TPB_{22})$ which is -2 times the gradient in rQAP. We let $r= 0.5- \frac{atan((iter-(maxiter/2)))}{\pi}$, so as the iteration counter, iter goes from $1 to $maxiter$, r goes from 1 to 0. This hybrid formulation will behave like rQAP2 for initial iterations and will start to behave like rQAP as $iter$ approaches $maxiter$.
