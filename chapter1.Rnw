...
In this thesis, we investigate how to perform inference in settings in which the data involve different modalities or views. For effective learning, data fusion that considers all views of these \emph{multiview} data settings is required to use the information available. We also require dimensionality reduction to address the problems associated with high dimensionality known as ``the curse of dimensionality''. We are interested in the type of information available in the multiview data that is essential for the inference task. We also want to determine the  principles to be used throughout the dimensionality reduction and data fusion steps for acceptable task performance. Our research focus is on exploring how these queries and their answers are relevant to particular data problems.

\section{Data Settings}


 It is a challenge to perform a tractable analysis on data obtained from disparate sources (such as multiple sensors). The increasing variety of sensor technologies and the large number of sensors introduce challenges but also hold promise for effective inference. One of our contributions is the development of well-defined simple settings that provide intuition about the right approaches to data fusion and lead to the development of practically useful inference methods.
...
We use the embedding approach  so  that we avoid the``curse of dimensionality'' with the low embedding dimension, allowing us to still use classic statistical tools. Additionally, the embeddings of dissimilarities from different conditions are required to be ``commensurate'' so that sensor measurements can be compared (\ie, the degree of (dis)similarity can be measured) or jointly used in inference. This is accomplished by maps $\rho_k,k=1,\ldots,K$ from measurement spaces $\Xi_k$ to a low-dimensional commensurate space $\mathcal{X}$, visualized in \autoref{fig:gen-model}. Learning these maps from data is an important part of our novel approach.
\label{sec:data}
...
\section{Dissimilarity representation\label{sec:dissim_repr}}
 ...
 \cite{duin2005dissimilarity} is an excellent resource that compiles the research on learning from dissimilarity-based representation. In the introduction, the authors clarify the distinction between statistical and structural (syntactic) pattern recognition, which was discussed previously in \cite{NadlerSmith1993}. The first subfield  of pattern recognition  addresses the analysis of features, which are lists of measured values for object attributes. The second subfield uses a relational view of objects for representation. In both cases,  the task of discrimination  can rely on distances (however they are defined). P\k{e}kalska and Duin  suggest that dissimilarity measures  are a natural bridge between these types of information, and their applicability to multiple settings motivates our use of dissimilarity representation in information fusion. 
...
\[  \begin{array}{cccc}
      ...
      \end{array}      
\]
To each pair of measurements $x_{ik},x_{jk}$ in the same space, we can assign a dissimilarity value $\delta_{ijk}=\delta\{x_{ik},x_{jk}\}$, which is possibly dependent on the space $\Xi_k$. We assume the dissimilarities to be nonnegative, symmetric, and 0 for $\delta\{x_{ik},x_{ik}\}$.  We exploit this training set of  dissimilarities to perform inference on the following exploitation task:

 Given the dissimilarities between  $K$ new measurements/observations ($\bm{y}_{k};k=1,\ldots,K$) and the previous $n$ objects under $K$ conditions, 
...
For the hypothesis testing problem, we are to compute the test statistics for the objects represented by the given dissimilarities. In order to compute the test statistics, it is necessary to find a collection of mappings (one from each condition) to a lower-dimensional space such that new observations from each condition are made commensurate when they are  mapped to this space . These mappings do not need to be well defined; they can be the results of embedding routines for a particular dataset. As long as the embedding of the dissimilarity results in a unique mapping, out-of-sample (OOS) embeddings could be adjoined to the embedding of in-sample dissimilarities.
