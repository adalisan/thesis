...
Consider the case where $\Delta$ is Euclidean. So there exist $\bm{x}_{i} \in \mathbf{R}^d$ such that for any pair $s,t \in \{1,\ldots,n\}$,  $\bm{x}_{s}$ and $\bm{x}_{t}$  $\delta_{st}=d(\bm{x}_{s}-\bm{x}_{t})$. If $\bm{x}_{i}$ satisfy $\delta_{st}=d(\bm{x}_{s}-\bm{x}_{t})$,  $\bm{x}_{i}+\bm{u}$ for a constant vector $\bm{u}$. To remove this ambiguity,   $\sum{\bm{x}_{i}}$ is set to $\bm{0}$. Then, the relation between the entries of $\Delta$ and $\bm{x}_{i}$ can be written as
\[
\delta_{st}^2= d(\bm{x}_{s},\bm{x}_{t})^2 = \|\bm{x}_{s}   \|^2 +   \|\bm{x}_{t}   \|^2   -   2  \bm{x}_{s} \cdot  \bm{x}_{t}   
\label{eq:diss_wrt_embed_coords}\]
Summing \ref{eq:diss_wrt_embed_coords} over $s$, over $t$ and then over $s$ and $t$, we get the following identities
\begin{align*}
...
\label{eq:diss_wrt_embed_coords_2}
\end{align*}
.
...
\label{eq:diss_wrt_embed_coords_3}
\end{align*}
...
\label{eq:diss_wrt_embed_coords_4}
\end{align*}


Summing the three equations 
in \ref{eq:diss_wrt_embed_coords_4} and replacing   $\|\bm{x}_{s}   \|^2 +\|\bm{x}_{t}\|^2$  in the original equation \label{eq:diss_wrt_embed_coords} with this sum
...
Let $X$ be a random $d$-dimensional vector and $\mu$ and $\Sigma$ be its covariance matrix. Then, for a given dimension $d'$, the successive maximization of  $Var[u_{i}^T (X-\mu)]= E[u_{i}^T (X-\mu)(X-\mu)^T u_i]=u_{i}^T\Sigma u_{i}$, where $u_{i}$ is a  $d$-dimensional unit vector ($u_i^Tu_i=1$)  and $E[u_{i}^T X X^T u_j]=0 , 1\leq j<i$), gives the \emph{principal} components of $X$. These principal components are the directions that has the maximum variance subject to the uncorrelatedness constraint  and the $d \times d'$-matrix $U$ whose columns are  $u_i, i=1,\ldots,d'$ is the projection matrix that captures the maximum variance from $X$. That is the coordinates of $UX$ are uncorrelated and have highest amount of variance for any linear subspace.
...
Suppose $\X$ is a $n \times d$ configuration matrix, representing the sample of $X$. The PCA solution is  
given by the eigenvalue decomposition of $\X^T \X=U\Lambda U^T$. The columns of $U$ are the principal directions and $U \Lambda^{\frac{1}{2}}$ gives  the principal coordinates.
This dimensionality reduction method gives the same solution as classical Multidimensional Scaling when the dissimilarity matrix is $\Delta =d(\X)$. In that case, $\Delta^2=  \1 \bm{y} +\bm{y}\1 -2 \X \X^T$ where $\bm{y}= \left(\X\X^T \right)_{(d)}$ is an n-dimensional vector that consists of  the diagonal of $\X\X^T$.
In classical MDS,  we would compute  $Z=  - \frac{1}{2} J\Delta^2 J$  which gives $ Z= \X\X^T$. So for the same embedding dimension, the two methods would give the same answer.
